...
static std::string genBaseSubfolder(std::uint64_t milliz) {
  ...
  std::strftime(tmFmtAry.data(), TIME_FMT_LEN, "%F-%T",
                std::localtime(&secondz));
...
Analysis::Analysis()
    : crystOut(kCRYST_OUT, false), specIn(kSPEC_IN, false),
      siOut(kSIPM_OUT, false), siEngOut(kSIPM_ENGS_OUT, false),
      scintOut(kSCINT_OUT, false), cfgOut(kCONFIG_OUT, false) {
...
std::uint32_t Analysis::currentRunNumber() { return runNumber; }
...
void Analysis::processHitCollection(const G4VHitsCollection *hc) {
  ...
  const auto *concreteHc =
      static_cast<const G4THitsCollection<VirtualHit> *>(hc);
...
void Analysis::saveCrystalHits(const std::vector<VirtualHit *> *vec) {
  const auto &igc = GlobalConfigs::instance();
  auto saveCrystPos = igc.configOption<bool>("save-scintillator-positions");
  auto saveEachHitEnergy = igc.configOption<bool>("save-each-cryst-hit-energy");
  auto saveEachHitMomentum =
      igc.configOption<bool>("save-each-cryst-hit-momentum");

  std::vector<G4double> deposits;
  std::vector<G4ThreeVector> positions, momenta;
  ...
  if (!saveEachHitEnergy) {
    ...
    for (std::size_t i = 0; i < positions.size(); ++i) {
      ...
      crystOut.file() << " (" << pos.x() / mm << ',' << pos.y() / mm << ','
                      << pos.z() / mm << ',' << e / keV << ')';
    }
  }

  if (saveEachHitMomentum) {
    for (const auto &momentum : momenta) {
      crystOut.file() << ' ' << momentum;
    }
  }
...
AnalysisFileWrapper::AnalysisFileWrapper(const G4String &fnPfx, bool isBinary)
    : isBinary(isBinary), fileNamePrefix(fnPfx) {}
